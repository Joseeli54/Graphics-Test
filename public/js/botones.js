function ocultar(){
    document.getElementById('debito').style.display = 'none';
    document.getElementById('boton1').style.display = 'inline';
    document.getElementById('boton2').style.display = 'none';

}

function mostrar(){
  document.getElementById('debito').style.display = 'block';
  document.getElementById('boton2').style.display = 'inline';
  document.getElementById('boton1').style.display = 'none';
  document.getElementById('credito').style.display = 'none';
  document.getElementById('paypal').style.display = 'none';
  document.getElementById('efectivo').style.display = 'none';
  document.getElementById('digital').style.display = 'none';
  document.getElementById('boton3').style.display = 'inline';
  document.getElementById('boton4').style.display = 'none';
  document.getElementById('boton5').style.display = 'inline';
  document.getElementById('boton6').style.display = 'none';
  document.getElementById('boton7').style.display = 'inline';
  document.getElementById('boton8').style.display = 'none';
  document.getElementById('boton9').style.display = 'inline';
  document.getElementById('boton10').style.display = 'none';
}

function ocultar1(){
    document.getElementById('credito').style.display = 'none';
    document.getElementById('boton3').style.display = 'inline';
    document.getElementById('boton4').style.display = 'none';
}

function mostrar1(){
  document.getElementById('credito').style.display = 'block';
  document.getElementById('boton4').style.display = 'inline';
  document.getElementById('boton3').style.display = 'none';
  document.getElementById('debito').style.display = 'none';
  document.getElementById('paypal').style.display = 'none';
  document.getElementById('efectivo').style.display = 'none';
  document.getElementById('digital').style.display = 'none';
  document.getElementById('boton1').style.display = 'inline';
  document.getElementById('boton2').style.display = 'none';
  document.getElementById('boton5').style.display = 'inline';
  document.getElementById('boton6').style.display = 'none';
  document.getElementById('boton7').style.display = 'inline';
  document.getElementById('boton8').style.display = 'none';
  document.getElementById('boton9').style.display = 'inline';
  document.getElementById('boton10').style.display = 'none';
}

function ocultar2(){
    document.getElementById('paypal').style.display = 'none';
    document.getElementById('boton5').style.display = 'inline';
    document.getElementById('boton6').style.display = 'none';
}
function mostrar2(){
    document.getElementById('paypal').style.display = 'block';
    document.getElementById('boton6').style.display = 'inline';
    document.getElementById('boton5').style.display = 'none';
    document.getElementById('debito').style.display = 'none';
    document.getElementById('credito').style.display = 'none';
    document.getElementById('efectivo').style.display = 'none';
    document.getElementById('digital').style.display = 'none';
    document.getElementById('boton1').style.display = 'inline';
    document.getElementById('boton2').style.display = 'none';
    document.getElementById('boton3').style.display = 'inline';
    document.getElementById('boton4').style.display = 'none';
    document.getElementById('boton7').style.display = 'inline';
    document.getElementById('boton8').style.display = 'none';
    document.getElementById('boton9').style.display = 'inline';
    document.getElementById('boton10').style.display = 'none';
}

function ocultar3(){
    document.getElementById('efectivo').style.display = 'none';
    document.getElementById('boton7').style.display = 'inline';
    document.getElementById('boton8').style.display = 'none';
}
function mostrar3(){
  document.getElementById('efectivo').style.display = 'block';
  document.getElementById('boton8').style.display = 'inline';
  document.getElementById('boton7').style.display = 'none';
  document.getElementById('debito').style.display = 'none';
  document.getElementById('credito').style.display = 'none';
  document.getElementById('paypal').style.display = 'none';
  document.getElementById('digital').style.display = 'none';
  document.getElementById('boton1').style.display = 'inline';
  document.getElementById('boton2').style.display = 'none';
  document.getElementById('boton3').style.display = 'inline';
  document.getElementById('boton4').style.display = 'none';
  document.getElementById('boton5').style.display = 'inline';
  document.getElementById('boton6').style.display = 'none';
  document.getElementById('boton9').style.display = 'inline';
  document.getElementById('boton10').style.display = 'none';
}

function ocultar4(){
    document.getElementById('digital').style.display = 'none';
    document.getElementById('boton9').style.display = 'inline';
    document.getElementById('boton10').style.display = 'none';
}
function mostrar4(){
    document.getElementById('digital').style.display = 'block';
    document.getElementById('boton10').style.display = 'inline';
    document.getElementById('boton9').style.display = 'none';
    document.getElementById('debito').style.display = 'none';
    document.getElementById('credito').style.display = 'none';
    document.getElementById('paypal').style.display = 'none';
    document.getElementById('efectivo').style.display = 'none';
    document.getElementById('boton1').style.display = 'inline';
    document.getElementById('boton2').style.display = 'none';
    document.getElementById('boton3').style.display = 'inline';
    document.getElementById('boton4').style.display = 'none';
    document.getElementById('boton5').style.display = 'inline';
    document.getElementById('boton6').style.display = 'none';
    document.getElementById('boton7').style.display = 'inline';
    document.getElementById('boton8').style.display = 'none';
}
